document.addEventListener('DOMContentLoaded', function() {
        // Toggle section for managing question sets
        document.getElementById('toggleQuestionSets').addEventListener('click', function() {
            toggleSection('questionSetsSection');
            fetchQuestionSets();
        });

        // Fetch question sets and populate the list
        function fetchQuestionSets() {
            fetch('/get_question_sets')
                ...
                .then(data => {
                    ...
                    data.question_sets.forEach(set => {
                        ...
                        listItem.innerHTML = `
                            ...
                            <button class="delete-button" data-id="${set.id}">Löschen</button>
                        `;
                        questionSetList.appendChild(listItem);
                    });
                    ...
                    deleteButtons.forEach(button => {
                        button.addEventListener('click', function() {
                            const questionSetId = this.getAttribute('data-id');
        ...
        function deleteQuestionSet(id) {
            const deleteMarkers = confirm('Möchten Sie auch alle zugehörigen Antworten (Marker) löschen?');
            const deleteQuestionSet = confirm('Sind Sie sicher, dass Sie dieses Fragenset löschen möchten?');

            if (deleteQuestionSet) {
                fetch(`/delete_question_set/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ delete_markers: deleteMarkers })
                })
                .then(response => {
                    if (response.ok) {
                        alert('Fragenset erfolgreich gelöscht.');
                        fetchQuestionSets(); // Refresh the list after deletion
                    } else {
                        alert('Fehler beim Löschen des Fragensets.');
                    }
                })
                .catch(error => console.error('Error deleting question set:', error));
            }
        }

        // Refresh button to fetch question sets again
        document.getElementById('refreshQuestionSets').addEventListener('click', fetchQuestionSets);
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Toggle section for managing categories and projects
        ...
        function fetchCategories() {
            fetch('/get_categories')
                ...
                .then(data => {
                    ...
                    categoryCount.textContent = `Anzahl von Kategorien: ${uniqueCategories.length}`;

                    uniqueCategories.forEach(category => {
                        const listItem = document.createElement('li');
                        listItem.innerHTML = `
                            Kategorie: <strong>${category}</strong> 
                            <button class="view-projects-button" data-category="${category}">Projekte anzeigen</button>
                            <button class="delete-category-button" data-category="${category}">Kategorie löschen</button>
                        `;
                        categoryList.appendChild(listItem);
                    });

                    // Add event listeners to view and delete buttons
                    const viewProjectsButtons = document.querySelectorAll('.view-projects-button');
        ...
        // Fetch projects of a specific category and populate the list
        function fetchProjectsByCategory(category) {
            fetch(`/get_projects_by_category/${category}`)
                .then(response => response.json())
                .then(data => {
                    ...
                    data.projects.forEach(project => {
                        ...
                        listItem.innerHTML = `
                            ...
                            <button class="view-project-button" data-id="${project.id}">Anzeigen</button>
                            <button class="delete-project-button" data-id="${project.id}">Löschen</button>
                        `;
                    ...
                    viewProjectButtons.forEach(button => {
                        button.addEventListener('click', function() {
                            ...
                            window.location.href = `/project/${projectId}`;  // Change the URL as per your routing
                        });
                    });
                    ...
                    toggleSection('projectsByCategorySection');
                })
                .catch(error => console.error('Error fetching projects by category:', error));
        }
            ...
            if (confirm('Sind Sie sicher, dass Sie dieses Projekt löschen möchten?')) {
                ...
                .then(response => {
                    if (response.ok) {
                        ...
                        fetchProjectsByCategory(category);  // Refresh the list after deletion
                    } else {
        ...
        // Delete all projects of a category
        function deleteCategory(category) {
            if (confirm(`Sind Sie sicher, dass Sie alle Projekte der Kategorie ${category} löschen möchten?`)) {
                fetch(`/delete_projects_by_category/${category}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    }
                ...
                .then(response => {
                    if (response.ok) {
                        alert(`Alle Projekte der Kategorie ${category} wurden erfolgreich gelöscht.`);
        ...
        // Refresh button to fetch categories again
        document.getElementById('refreshCategories').addEventListener('click', fetchCategories);
    });
...
function updateCommentCountDisplay() {
    ...
    document.getElementById('toggleUsers').addEventListener('click', function() {
      ...
      toggleSection('questionsSection');
    });
		   document.getElementById('toggleBaustellen').addEventListener('click', function() {
      toggleSection('baustellenSection');
    });
    ...
    function resizeCharts() {
      for (let chartId in chartInstances) {
        if (chartInstances.hasOwnProperty(chartId)) {
          ...
          console.debug("Resized chart:", chartId);
        }
      }
      console.debug("All charts resized.");
    }
    ...
    function setupPercentageToggle() {
      ...
      togglePercentageBtn.addEventListener('click', () => {
        ...
        console.log(`Toggle PercentAlter: Now showing ${showPercentage ? 'percentages' : 'numbers'}`);
        togglePercentageBtn.textContent = showPercentage ? 'Zahlen anzeigen' : '% anzeigen';
...
function fetchAndRenderEngagedProjectsChart(threshold) {
    fetch(`/get_engaged_projects?threshold=${threshold}`).then(response => response.json()).then(data => {
        ...
        // Check if the chart instance exists and has the destroy method
        if (window.engagedProjectsChart && typeof window.engagedProjectsChart.destroy === 'function') {
            window.engagedProjectsChart.destroy();
        }

        window.engagedProjectsChart = new Chart(ctx, {
            type: 'pie',
            data: {
            ...
            options: {
                ...
                plugins: {
                    ...
                    legend: {
                        ...
                        labels: {
                            font: {
                                size: 24, // Significantly increased size
                                weight: 'bold',
                                color: '#000000'
                            }
                        }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let initialStartDateSubmission = new Date('{{ getEarliestSubmissionDate }}');
  ...
  let initialStartDateActivity = new Date('{{ getEarliestInteractionDate }}');
  ...
  let initialStartDateViews = new Date('{{ getEarliestProjectViewDate }}');
  ...
  let initialStartDateUniqueViews = new Date('{{ getEarliestWebsiteViewDate }}');
  document.getElementById('start-date').value = initialStartDateUniqueViews.toISOString().split('T')[0];
  document.getElementById('end-date').value = initialEndDateSubmission.toISOString().split('T')[0]; // This should probably reference 'initialEndDateSubmission' directly if it's intentional
  fetchDataForRange();
});
    document.addEventListener("DOMContentLoaded", function() {
      fetch('/get_unique_viewers_data').then(response => response.json()).then(data => {
        ...
        // Get current date and time and log it to the console
        const now = new Date();
        const dateString = now.toLocaleString();
        console.log("Unique Viewers Chart loaded with data as of " + dateString, data);
      }).catch(error => console.error('Error:', error));
    });
    document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchDataForRange() {
      ...
      console.log("Fetching data for range:", startDate, endDate); // Debugging
    ...
    `;
      console.log("Displayed viewer stats:", {
        maxViewersDate,
        maxViewers,
        totalViewers
      });
    ...
    document.getElementById('deleteMapObjectsByDateBtn').addEventListener('click', function() {
      ...
      fetch('/preview_delete_map_objects_by_date', {
        ...
        if (data.success) {
          console.log(`Anzahl der zu löschenden Kartenobjekte: ${data.mapObjects.length}`);
          data.mapObjects.forEach(obj => console.log(`Beschreibung: ${obj.descriptionwhy}`));
          if (confirm(`Möchten Sie alle Kartenobjekte zwischen ${fromDate} und ${toDate} löschen?`)) {
            deleteMapObjects(fromDate, toDate);
          }
        } else {
          console.error('Fehler beim Abrufen von Kartenobjekten:', data.error);
    ...
    function deleteMapObjects(fromDate, toDate) {
      console.log(`Löschung von Kartenobjekten angefordert für den Zeitraum: ${fromDate} bis ${toDate}`);
      fetch('/delete_map_objects_by_date', {
        ...
        if (data.success) {
          console.log(`Kartenobjekte erfolgreich gelöscht für den Zeitraum: ${fromDate} bis ${toDate}`);
          alert('Kartenobjekte erfolgreich gelöscht.');
        } else {
          console.error('Fehler beim Löschen von Kartenobjekten:', data.error);
...
function addToImportantProjects(project) {
    ...
    if (!list) {
        console.error('Important projects list container not found.');
        return;
    }
...
addProjectToDOM({id: 1, name: "Project 1"});
...
<!-- Add this script at the bottom of admintools.html or in a separate JS file included in the page -->
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.submit-answer').forEach(button => {
        button.addEventListener('click', function() {
            ...
            const answerText = form.querySelector('textarea[name="answer_text"]').value;
            ...
            .then(data => {
                if (data.success) {
                    ...
                    updateQuestionWithAnswer(questionId, answerText, new Date().toISOString());
                    // Hide the form and button to prevent duplicate submissions
...
function updateQuestionWithAnswer(questionId, answerText, answerDate) {
    const questionDiv = document.getElementById('question-' + questionId);
    if (questionDiv) {
        let answerDiv = questionDiv.querySelector('.answer');
        if (!answerDiv) {
            answerDiv = document.createElement('div');
            answerDiv.classList.add('answer');
            questionDiv.appendChild(answerDiv);
        }
        answerDiv.innerHTML = `<strong>Antwort:</strong> ${answerText}`;

        let answerDateDiv = questionDiv.querySelector('.answer-date');
        if (!answerDateDiv) {
            answerDateDiv = document.createElement('div');
            answerDateDiv.classList.add('answer-date');
            questionDiv.appendChild(answerDateDiv);
        }

        // Correct the date formatting here to match the desired format
        let formattedDate = new Date(answerDate).toLocaleString('de-DE', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
        }).replace(',', ''); // Ensure consistent formatting

        answerDateDiv.innerHTML = `<strong>Am:</strong> ${formattedDate}`;
    }
}
...
function updateQuestionsCount() {
    const debugCountElement = document.getElementById('questions-count');
    if (debugCountElement) {
        // Assuming you have a way to get the updated count, for demonstration assuming it's through an API or stored in JS variable
        const updatedCount = document.querySelectorAll('.project-thumbnail').length;
        debugCountElement.innerText = `Debug: XD ${updatedCount} questions.`;
    } else {
        console.error("Failed to find the count element.");
    }
}


document.getElementById('partial-container').addEventListener('click', function(event) {
...
function confirmDeleteBaustelle(baustelleId) {
    if (confirm('Möchten Sie diese Baustelle wirklich löschen?')) {
        fetch(`/delete_baustelle/${baustelleId}`, {
            method: 'DELETE',
        })
        .then(response => {
            if (response.ok) {
            ...
            throw new Error('Failed to delete baustelle.');
        })
        .then(data => {
            document.getElementById(`baustelle-${baustelleId}`).remove();
            // Update the Partizipative_Planung_Fragen_Baustelle count
            updateBaustellenCount();
            alert('Baustelle erfolgreich gelöscht.');
        })
        .catch(error => {
            console.error('Error:', error);
            alert('Fehler beim Löschen der Baustelle.');
        });
    }
}

function updateBaustellenCount() {
    const baustellenContainer = document.getElementById('Partizipative_Planung_Fragen_Baustelle-list-container');
    const Partizipative_Planung_Fragen_Baustelle = baustellenContainer.querySelectorAll('.project-thumbnail');
    const debugCountElement = document.getElementById('Partizipative_Planung_Fragen_Baustelle-count');
    debugCountElement.innerHTML = `Baustellen: ${Partizipative_Planung_Fragen_Baustelle.length}`;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const data = {
        ...
        datasets: [{
            data: [{{ questions_stats.answered | tojson }}, {{ questions_stats.unanswered | tojson }}],
            backgroundColor: ['#021c4c', '#e82b10'],
            borderColor: ['#021c4c', '#e82b10'],
            borderWidth: 1
        }]
    };
    const options = {
        ...
        plugins: {
            ...
            tooltip: {
                callbacks: {
                    label: function(context) {
                        ...
                        const percentage = ((context.parsed / total) * 100).toFixed(0); // Show percentage without decimal
            ...
            datalabels: {
                ...
                formatter: (value, context) => {
                    return value; // Just show the value inside the chart
                }
            }
        },
        animation: {
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize global variable to hold our chart instance
    window.questionChart = null; // This will later store our chart instance

    // Set the default option to "Alles" on page load
    document.getElementById('baustelleFilter').value = 'all';

    // Load initial chart and stats for "Alles"
    updateQuestionsStats(); // This will fetch and display the stats for all Baustellen
});

// Function to fetch data based on selected Baustelle and update chart and stats
function updateQuestionsStats() {
    const baustelleId = document.getElementById('baustelleFilter').value;
    let url = baustelleId === 'all' ? `/get_questions_stats_for_all_baustelles` : `/get_questions_stats_by_baustelle?baustelle_id=${baustelleId}`;
    fetch(url)
        .then(response => response.json())
        .then(data => {
            document.getElementById('answeredPercentage').innerHTML = `<strong>${data.answered_percentage.toFixed(0)} %</strong> der Fragen wurden beantwortet.`;
            
            const ctx = document.getElementById('questionChart').getContext('2d');
            if (window.questionChart !== null) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to show the nav-overlay
    function showNavOverlay() {
        if (window.innerWidth <= 1080) { // Only show on mobile
            navOverlay.style.display = 'block';
            console.debug('nav-overlay opened.');
        }
    }
    
    // Function to hide the nav-overlay
    function hideNavOverlay() {
        navOverlay.style.display = 'none';
        console.debug('nav-overlay closed.');
    }
    
    // Toggle the nav-overlay when the hamburger button is clicked
    ...
    // Close the nav-overlay when the close button is clicked
    closeOverlayBtn.addEventListener('click', function(event) {
        event.stopPropagation(); // Prevent the click from propagating to the nav-overlay
        hideNavOverlay();
    });
    
    // Listen for clicks on the nav-overlay to close it when clicking outside of #nav-links
    navOverlay.addEventListener('click', function(event) {
        if (event.target === navOverlay) { // Check if the clicked element is the nav-overlay itself
            hideNavOverlay();
        }
    });
    
    // Adjust the display of nav-overlay on resize
    window.addEventListener('resize', function() {
        if (window.innerWidth > 1080) {
            navOverlay.style.display = 'block'; // Always show on desktop
        } else {
            navOverlay.style.display = 'none'; // Hide by default on mobile
        }
    });
});
    ...
    fetch('/log_view').then(response => response.json()).then(data => console.debug(data.message)).catch(console.error);
